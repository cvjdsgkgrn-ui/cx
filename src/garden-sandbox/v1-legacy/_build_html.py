# Strategy: instead of fixing the self-modifying build_step1.py,
# write a single Python script that directly generates index.html
# with HTML+CSS+JS all in one go
# This avoids all the quote-escaping madness

# First, let me generate the HTML/CSS part separately
import json
...
# Build HTML with proper encoding
lines = []
...
lines.append('<title>' + esc("苏州园林 AI 沙箱") + '</title>')
lines.append('<style>')

# CSS - compact single line
css = (
    ...
    '#knowledge-card{background:rgba(15,52,96,0.7);border-radius:8px;padding:5px 10px;font-size:11px;color:#bbb;line-height:1.5;border-left:3px solid #e2b04a;flex-shrink:0;cursor:default;transition:all .3s;min-height:32px;display:flex;align-items:center}'
    '#knowledge-card:hover{background:rgba(78,205,196,0.08);border-left-color:#4ecdc4;color:#ddd}'
    ...
    '.weather-btn{min-width:32px;height:32px;border-radius:8px;border:1px solid rgba(255,255,255,0.1);cursor:pointer;font-size:14px;background:rgba(255,255,255,0.03);display:flex;flex-direction:column;align-items:center;justify-content:center;transition:all .15s;padding:2px 4px;gap:0}'
    '.w-label{display:block;font-size:7px;line-height:1;margin-top:1px;color:#fff}'
    ...
    '#pov-panel{background:#0f3460;border-radius:10px;position:relative;overflow:hidden;border:1px solid rgba(255,255,255,0.08);flex:1;min-height:0}'
    ...
    '.speech-bubble{position:absolute;background:rgba(0,0,0,0.88);color:#fff;padding:4px 8px;border-radius:6px;font-size:10px;max-width:180px;text-align:center;animation:bubbleFade 3.5s ease-out forwards;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;line-height:1.3;pointer-events:none}'
    ...
    '#quick-row{position:absolute;bottom:16%;left:50%;transform:translateX(-50%);display:flex;gap:4px;padding:4px 6px;background:rgba(0,0,0,0.6);border-radius:6px;flex-wrap:wrap;z-index:6;backdrop-filter:blur(4px)}'
    ...
    '#add-char-dialog .btn-cancel{background:rgba(255,255,255,0.1);color:#fff}'
    ...
    '#settings-btn{position:fixed;top:10px;left:10px;z-index:101;width:32px;height:32px;border-radius:50%;border:1px solid rgba(255,255,255,0.12);background:rgba(0,0,0,0.5);color:#fff;font-size:16px;cursor:pointer;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(6px);transition:all .15s}'
    '#settings-btn:hover{background:rgba(78,205,196,0.15);border-color:#4ecdc4;color:#4ecdc4}'
    '#settings-panel{position:fixed;top:46px;left:10px;z-index:100;background:rgba(15,20,35,0.95);border:1px solid rgba(255,255,255,0.1);border-radius:8px;padding:12px;backdrop-filter:blur(10px);display:none;min-width:240px}'
...
# HTML Body
body = '<body>'
body += '<button id="settings-btn" title="' + esc("设置") + '">⚙</button>'
body += '<div id="settings-panel"><div class="settings-inner">'
body += '<label for="api-key">' + esc("API Key") + '</label><input id="api-key" type="password" placeholder="sk-...">'
body += '<label for="api-url">' + esc("API URL") + '</label><input id="api-url" value="https://api.deepseek.com/v1/chat/completions">'
body += '<div class="settings-btns"><button id="api-save">' + esc("保存") + '</button><span id="api-status"></span></div>'
body += '</div></div>'
body += '<div id="app">'
body += '<div id="map-panel"><div id="map-three-top"></div><div id="knowledge-card">' + esc("点击角色标签查看匠人技艺知识") + '</div><div id="map-canvas-bottom"><canvas id="map-canvas"></canvas></div></div>'
body += '<div id="right-main">'
body += '<div id="character-tabs"></div>'
body += '<div id="top-bar">'
body += '<div id="weather-group">'
body += '<button class="weather-btn active" data-w="day" title="' + esc("白天") + '">☀<span class="w-label">' + esc("白天") + '</span></button>'
body += '<button class="weather-btn" data-w="dusk" title="' + esc("黄昏") + '">🌅<span class="w-label">' + esc("黄昏") + '</span></button>'
body += '<button class="weather-btn" data-w="night" title="' + esc("夜晚") + '">🌙<span class="w-label">' + esc("夜晚") + '</span></button>'
body += '<button class="weather-btn" data-w="rain" title="' + esc("雨天") + '">🌧<span class="w-label">' + esc("雨天") + '</span></button>'
body += '</div>'
body += '<div id="status-mini"><span class="s-item"><span class="s-label">' + esc("角色") + '</span><span class="s-val" id="st-name">-</span></span><span class="s-item"><span class="s-label">' + esc("身份") + '</span><span class="s-val" id="st-role">-</span></span><span class="s-item"><span class="s-label">' + esc("年龄") + '</span><span class="s-val" id="st-age">-</span></span><span class="s-item" id="st-mood">😐</span></div>'
body += '<button id="timeline-btn">📋 ' + esc("记录") + '</button>'
body += '</div>'
body += '<div id="info-panel"><div id="text-stream"></div></div>'
body += '<div id="pov-panel"><div id="pov-label"></div><div id="pov-container"></div><div id="speech-bubbles"></div><div id="quick-row">' + "<button class='quick-btn' data-target='-10,-8'>🏯 " + esc("去亭子") + "</button><button class='quick-btn' data-target='10,-4'>🚪 " + esc("去月洞门") + "</button><button class='quick-btn' data-target='5,-4'>💧 " + esc("去水池") + "</button><button class='quick-btn' data-target='-14,7'>🪨 " + esc("去假山") + "</button>" + '</div></div>'
body += '</div></div>'
body += '<div id="add-char-dialog"><div class="dialog-box"><h3>' + esc("添加新角色") + '</h3>'
body += '<label for="new-char-name">' + esc("角色名") + '</label><input id="new-char-name" placeholder="' + esc("如：老李") + '">'
body += '<label for="new-char-role">' + esc("身份") + '</label><input id="new-char-role" placeholder="' + esc("如：石匠、园丁、诗人...") + '">'
body += '<label for="new-char-gender">' + esc("性别") + '</label><select id="new-char-gender"><option value="male">' + esc("男性") + '</option><option value="female">' + esc("女性") + '</option></select>'
body += '<label for="new-char-age">' + esc("年龄") + '</label><input id="new-char-age" type="number" value="30" min="8" max="90">'
body += '<label for="new-char-color">' + esc("颜色") + '</label><div class="color-pick-row"><input type="color" id="new-char-color" value="#e07b3c"><input type="text" id="new-char-hex" class="hex-input" placeholder="#e07b3c"></div>'
body += '<label for="new-char-persona">' + esc("人设描述") + '</label><textarea id="new-char-persona" placeholder="' + esc("如：香山帮石匠，五十多岁，太湖石在他手里能变成活物...") + '"></textarea>'
body += '<div class="btn-row"><button class="btn-random" id="btn-random-char">' + esc("随机生成") + '</button><button class="btn-cancel" id="btn-cancel-add">' + esc("取消") + '</button><button class="btn-create" id="btn-create-char">' + esc("创建角色") + '</button></div>'
body += '</div></div>'
body += '<div id="timeline-panel"><h4>📋 ' + esc("事件时间线") + ' <button class="tl-close" id="tl-close">✕</button></h4><div id="tl-list"></div></div>'
body += '<div id="hint-bar">' + esc("3D 环绕鸟瞰 | 右键地图导航 | 点击标签切换角色") + '</div>'

lines.append(body)

# Import map
lines.append('<script type="importmap">')
lines.append('{ "imports": { "three": "https://unpkg.com/three@0.160.0/build/three.module.js", "three/addons/": "https://unpkg.com/three@0.160.0/examples/jsm/" } }')
lines.append('</script>')

# Save HTML part only (steps 2-4 will append JS)
with open(OUT, "w", encoding="utf-8-sig", newline="\n") as f:
    f.write("\n".join(lines))

print("HTML part written OK, size:", len("\n".join(lines)))
