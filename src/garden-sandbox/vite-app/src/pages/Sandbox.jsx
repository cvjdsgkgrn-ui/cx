...
const DEFAULT_API_KEY = "sk-0d0e8cb1b88140f9a09de82af96922e8"
const DEFAULT_API_URL = "/api/deepseek/v1/chat/completions"
    ...
    if (mapMode === "custom") {
      try {
        ...
        if (raw) {
          ...
          setSceneData(SCENE_SIZE, tileData)
    ...
    const tick = () => {
      ...
      for (const c of chars) {
        ...
        if (c.observeTimer<=0) {
          ...
          generateFeeling(c, pickRandom(landmarks), DEFAULT_API_KEY, DEFAULT_API_URL)
        ...
        for (const other of chars) {
          ...
          if (d<CONVERSE_DIST && (now-(c.lastConverse[other.id]||0))>CONVERSE_COOLDOWN_MS) {
            ...
            generateDialogue(c, other, DEFAULT_API_KEY, DEFAULT_API_URL).then(t=>{
  ...
  return (
    <div style={{display:"grid",gridTemplateColumns:"30fr 70fr",height:"100vh",gap:4,padding:4,background:"#1a1a2e",fontFamily:"Microsoft YaHei,PingFang SC,sans-serif"}}>
      <div style={{display:"flex",flexDirection:"column",gap:3,minWidth:0}}>
        ...
        <div onClick={updateKnowledge} style={{background:"rgba(15,52,96,0.7)",borderRadius:6,padding:"4px 8px",fontSize:11,color:"#aaa",borderLeft:"2px solid #e2b04a",flexShrink:0,cursor:"pointer",maxHeight:44,overflow:"hidden"}}>
          <b>{knowledgeCard.title}</b>: {knowledgeCard.text.substring(0,50)}...
        </div>
      </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:4,minWidth:0}}>
        ...
        {showSettings && (
          <div style={{background:"rgba(15,20,35,0.95)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:12,flexShrink:0}}>
            ...
            <input type="password" defaultValue={DEFAULT_API_KEY} readOnly style={{width:"100%",padding:"6px 8px",borderRadius:4,border:"1px solid #444",background:"#0a0f1e",color:"#888",fontSize:11}}/>
            ...
            <div style={{color:"#4ecdc4",fontSize:10,marginTop:6}}>API Key 已内置</div>
          </div>
        )}
      ...
      {showAddChar&&(
        <AddCharacterDialog onAdd={addCharacter} onClose={()=>setShowAddChar(false)}
          apiKey={DEFAULT_API_KEY} apiUrl={DEFAULT_API_URL}/>
      )}
    </div>
  )
}
