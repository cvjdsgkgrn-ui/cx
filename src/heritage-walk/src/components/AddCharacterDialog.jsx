...
function randomPersona(name, role) {
  const templates = [
    `一个热爱扬州老城区的${role}，对名人故居如数家珍。`,
    `本地长大的${role}，对故居的一砖一瓦都有感情。`,
    `从外地慕名而来的${role}，被故居的历史感震撼到了。`,
    `${role}，性格开朗健谈，喜欢和遇到的每个人聊天。`,
    `${role}，安静内敛，喜欢独自感受故居的每个角落。`
  ]
...
export default function AddCharacterDialog({ onAdd, onClose, apiKey, apiUrl }) {
  ...
  const handleCreate = useCallback(() => {
    ...
    const p = persona.trim() || randomPersona(n, r)
    onAdd({ name: n, role: r, color, gender, age, persona: p })
    onClose()
  }, [name, role, color, gender, age, persona, onAdd, onClose])
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 14, justifyContent: "flex-end" }}>
          ...
          <button onClick={handleCreate}
            style={{ padding: "6px 16px", borderRadius: 5, border: "none", cursor: "pointer", background: "#4ecdc4", color: "#111", fontWeight: "bold", fontSize: 12 }}>
            创建
          </button>
        </div>
      </div>
