...
async function runKeyPic(id) {
    ...
    if (genMode === 'volcano') {
        ...
        await fetch('/api/image_gen_google', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ node_id: id, api_key: apiKey, model, prompt, image_config: imgCfg, ref_image_url: refImg ? refImg.url : undefined })
        });
    } else if (genMode === 'api') {
        const body = {
            node_id: id,
        ...
        wfStr = wfStr.replace('{{positive_prompt}}', prompt).replace('{{negative_prompt}}', shot.negative_prompt || 'blurry, low quality').replace('{{width}}', '1920').replace('{{height}}', '1080');
        let wf;
